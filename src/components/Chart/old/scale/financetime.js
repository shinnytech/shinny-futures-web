...
export default (
  ...
  function financetime(tickMethods, genericFormat, index, domain, padding, outerPadding, zoomLimit, closestTicks, zoomable) {
    ...
    class scale {
      ...
      static ticks(interval, steps) {
        ...
        const intervalRange = interval.every(steps).range(visibleDomain[0], +visibleDomain[visibleDomain.length-1]+1);
       ...
       */
      static tickFormat() {
        return date => tickState.tickFormat(date);
      }
    }
    ...
    function tickMethod(visibleDomain, indexDomain, count) {
      ...
      const // Determine whether we're showing daily or intraday data
      ...
      const // Adjust the target based on proportion of domain that is visible
        target = visibleDomainExtent/k;

      const i = d3_bisect(tickSteps, target);
  ...
  const intradayFormat = d3_v3_multi_shim([
    ...
    [d3_time.timeFormat('%I %p'), () => true]
  ]);

  const genericFormat = [d3_time.timeSecond, 1, d3_v3_multi_shim([
    [d3_time.timeFormat(':%S'), d => d.getSeconds()],
    [d3_time.timeFormat('%I:%M'), d => d.getMinutes()],
  ...
  const intradayFormatUtc = d3_v3_multi_shim([
    ...
    [d3_time.utcFormat('%I %p'), () => true]
  ]);

  const genericFormatUtc = [d3_time.timeSecond, 1, d3_v3_multi_shim([
    [d3_time.utcFormat(':%S'), d => d.getUTCSeconds()],
    [d3_time.utcFormat('%I:%M'), d => d.getUTCMinutes()],
