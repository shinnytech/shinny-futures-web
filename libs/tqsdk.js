  ...
  let handler = {
    get: function (target, property, receiver) {
      if (!isNaN(property)) {
        ...
        if (i < 0) { return NaN }
        // i = target.length + i;
        if (item_func) { return item_func(target[i]) } else { return target[i] }
...
class DataManager {
      ...
      switch (typeof value) {
        case 'object':
          if (value === null) {
            ...
            if (target.left_id == undefined || isNaN(target.left_id) || Object.keys(value)[0] < target.left_id) { target.left_id = Number(Object.keys(value)[0]) }
  ...
  set_default (default_value, ...path) {
    ...
    for (let i = 0; i < path.length; i++) {
      if (!(path[i] in node)) { node[path[i]] = (i + 1 === path.length) ? default_value : {} }
      node = node[path[i]]
    }
    return node
  }
  ...
  get (...path) {
    let node = this.datas
    for (let i = 0; i < path.length; i++) {
      if (!(path[i] in node)) { return undefined }
      node = node[path[i]]
    }
    return node
  }
...
class TQSDK {
  ...
  on_rtn_data (message_rtn_data) {
    // 更新到数据存储区
    if (message_rtn_data.aid == 'rtn_data') { this.dm.on_rtn_data(message_rtn_data) }
  }
