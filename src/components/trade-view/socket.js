/* eslint-disable no-unused-vars,camelcase,no-undef,no-redeclare,no-undef-init,no-tabs */
const delay = timeout => {
...
const Io = {
  ...
  judge_data_complete: function (symbol, dur_nano, left_kline_id, view_width) {
    try {
      if (typeof (TQ.dm.datas.klines[symbol][dur_nano]['data']) === 'undefined') { return false }
    } catch (err) {
      return false
    }
    ...
    if (typeof (symbol_map_data) === 'undefined') { this.time_map[symbol + dur_nano] = {} }
    var symbol_map_data = {}
    ...
    if (judge_start_pos > 0) { // TODO 应该更仔细地去判断是>还是>=，细节问题
      for (var i = 0; i < view_width; i++) {
        ...
        if (this.has_key(symbol_map_data, id_temp)) { continue }
    ...
    if (judge_data[left_id] === undefined || judge_data[last_id] === undefined) { return false }

    this.kline_id_map[symbol + dur_nano] = { 'left_id': left_id,
      'left_id_datetime': judge_data[left_id]['datetime'] / 1000000000,
      'last_id': last_id,
      'last_id_datetime': judge_data[last_id]['datetime'] / 1000000000 }
  ...
  process_kline_data: function (symbol, dur_nano, left_kline_id, view_width) {
    ...
    for (var i = 0; i < view_width; i++) {
      if (typeof (data_temp) === 'undefined') { continue }
  ...
  process_kline_data_from_to: function (symbol, dur_nano, from, to) {
    ...
    if ((from - left_id_datetime) / (last_id_datetime - left_id_datetime) < 0.5) { var direction = 'left' } else { var direction = 'right' }
    ...
    if (direction === 'left') {
      for (var pos_temp = left_id; pos_temp <= last_id; pos_temp++) {
        ...
        if (date_judge_temp < from) { last_lagerest_left_date = date_judge_temp } else if ((date_judge_temp >= from) && (date_judge_temp <= to)) { return_data.push({ 'time': date_temp, 'open': data_temp.open, 'close': data_temp.close, 'low': data_temp.low, 'high': data_temp.high, 'volume': data_temp.volume }) } else if (date_judge_temp > to) { break }
      }
    } else {
      for (var pos_temp = last_id; pos_temp >= left_id; pos_temp--) {
        ...
        // console.log( date_judge_temp );
        // console.log( (date_judge_temp>=from)&&(date_judge_temp<=to) );
        if ((date_judge_temp >= from) && (date_judge_temp <= to)) { return_data.push({ 'time': date_temp, 'open': data_temp.open, 'close': data_temp.close, 'low': data_temp.low, 'high': data_temp.high, 'volume': data_temp.volume }) } else if (date_judge_temp < from) {
          last_lagerest_left_date = date_judge_temp
          break
        }
      }
    }
    // TODO:这边逻辑有小问题，关于每时刻右边动态刷新的逻辑判断
    if (return_data.length === 0 && typeof (last_lagerest_left_date) !== 'undefined') { return { 'kline': return_data, 'nextTime': last_lagerest_left_date, 'from': from, 'to': to } } else { return { 'kline': return_data } }
  },
