...
class Datafeeds {
  constructor(mgr, updateFrequency) {
    ...
    this._barsPulseUpdater = new DataPulseUpdater(this, updateFrequency || 10 * 1000)
      ...
      this._barsPulseUpdater.showStcok = true;
    ...
    /**
     * 改变showStcok状态，退出K线状态时就不需要实时刷新数据了
     */
  change_showStcok_status(status){
        this._barsPulseUpdater.showStcok = status;
    }
  /**
   * 默认配置
   */
  ...
  resolveSymbol(symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
    ...
    if (!this._configuration.supports_group_request) {
      setTimeout(function () {
        ...
        const data = that._mgr.resolveTVSymbol(symbolName ? symbolName : '')
        if (data) {
          onResultReady(data)
        } else {
  ...
  getBars(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
    ...
    this._mgr.getBars(symbolInfo.ticker, resolution, rangeStartDate, rangeEndDate, onLoadedCallback)
  }
     ...
     * @param {*Function 回调函数} onErrorCallback
     */
    getBars_update(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback){
        const onLoadedCallback = function (data) {
        ...
        this._mgr.getBars_update(symbolInfo.ticker, resolution, rangeStartDate, rangeEndDate, onLoadedCallback)
    }
    ...
    const update = function () {
      ...
      for (let listenerGUID in that._subscribers) {
        ...
        (function (_subscriptionRecord) {
          if( !window.store_obj.state.showStcok_status ){
              console.log("showStcok is false. Update data stop")
              return;
          }
          that._datafeed.getBars_update(_subscriptionRecord.symbolInfo, resolution, datesRangeLeft, datesRangeRight, function (bars) {
            that._requestsPending--
  ...
  periodLengthSeconds(resolution, requiredPeriodsCount) {
    ...
    if (resolution === 'D') {
      daysCount = requiredPeriodsCount
        ...
        //daysCount = requiredPeriodsCount * parseInt(resolution.replace("D",""))
        daysCount = requiredPeriodsCount
